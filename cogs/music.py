...
class Music(commands.Cog):
    ...
    async def handle_play(self, guild_id: int, author: discord.Member, send, search: str):
        voice_client = self.voice_clients.get(guild_id)
        if not voice_client or not voice_client.is_connected():
            if not author.voice or not author.voice.channel:
                await send("음성 채널에 먼저 참여해주세요.")
                return
        ...
        try:
            ...
            await send("곡 정보를 가져오는 중 오류가 발생했습니다.")
            return
        ...
        if not voice_client.is_playing() and not voice_client.is_paused():
            ...
            embed = await self.build_queue_embed(guild_id, 0)
            view = Music.QueueNavView(self, guild_id)
    ...
    async def set_music_channel_slash(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        ...
        with open(self.channel_path, "w", encoding="utf-8") as f:
            json.dump({str(k): v for k, v in self.music_channel_per_guild.items()}, f, ensure_ascii=False, indent=4)
        await interaction.response.send_message(f"✅ 이 서버의 뮤직 채널이 {target.mention}(으)로 설정되었습니다.", ephemeral=True)
    ...
    @app_commands.command(name="join", description="음성 채널에 참여합니다.")
    async def join_slash(self, interaction: discord.Interaction):
        if not self.is_music_channel(interaction):
            await interaction.response.send_message("먼저 `/setmusicchannel` 명령어로 이 서버의 뮤직 채널을 지정해주세요.", ephemeral=True)
            return
        ...
        if existing and existing.is_connected():
            await interaction.response.send_message("이미 음성 채널에 연결되어 있습니다.", ephemeral=True)
            return
        self.voice_clients[interaction.guild.id] = await vc.connect()
        await interaction.response.send_message("음성 채널에 연결했습니다.", ephemeral=True)
    ...
    @app_commands.command(name="leave", description="음성 채널에서 나갑니다.")
    async def leave_slash(self, interaction: discord.Interaction):
        ...
        self.guild_queues.pop(interaction.guild.id, None)
        self.now_playing_msg.pop(interaction.guild.id, None)
        await interaction.response.send_message("음성 채널에서 나왔습니다.", ephemeral=True)
    ...
    async def play(self, ctx: commands.Context, *, search: str):
        ...
        await self.handle_play(ctx.guild.id, ctx.author, sender, search)

    @app_commands.command(name="play", description="음악을 재생합니다.")
    @app_commands.describe(search="검색어 또는 YouTube 링크")
    async def play_slash(self, interaction: discord.Interaction, search: str):
        ...
        await self.handle_play(interaction.guild.id, interaction.user, sender, search)
    ...
    async def skip_slash(self, interaction: discord.Interaction):
        ...
        if not voice_client or not voice_client.is_connected() or not voice_client.is_playing():
            ...
            return
        voice_client.stop()
        await interaction.response.send_message("⏭ 다음 곡으로 건너뜁니다.", ephemeral=True)
    ...
    async def pause_slash(self, interaction: discord.Interaction):
        ...
        if voice_client and voice_client.is_playing():
            voice_client.pause()
            await interaction.response.send_message("⏸ 재생 일시정지되었습니다.", ephemeral=True)
    ...
    async def resume_slash(self, interaction: discord.Interaction):
        ...
        if voice_client and voice_client.is_paused():
            voice_client.resume()
            await interaction.response.send_message("▶ 재생을 다시 시작합니다.", ephemeral=True)
    ...
    async def queue_slash(self, interaction: discord.Interaction):
        if not self.is_music_channel(interaction):
            await interaction.response.send_message("먼저 `/setmusicchannel` 명령어로 이 서버의 뮤직 채널을 지정해주세요.", ephemeral=True)
            return
        embed = await self.build_queue_embed(interaction.guild.id, 0)
        view = Music.QueueNavView(self, interaction.guild.id)
    ...
    @app_commands.command(name="stop", description="재생을 모두 중지합니다.")
    async def stop_slash(self, interaction: discord.Interaction):
        ...
        self.voice_clients.pop(interaction.guild.id, None)
        self.now_playing_msg.pop(interaction.guild.id, None)
        await interaction.response.send_message("⏹ 재생을 중단하고 음성 채널에서 나왔습니다.", ephemeral=True)
    ...
    @app_commands.command(name="saveplaylist", description="현재 대기열을 저장합니다.")
    @app_commands.describe(name="플레이리스트 이름")
    async def save_playlist_slash(self, interaction: discord.Interaction, name: str):
        if not self.is_music_channel(interaction):
            await interaction.response.send_message("먼저 `/setmusicchannel` 명령어로 이 서버의 뮤직 채널을 지정해주세요.", ephemeral=True)
            return
        queue = self.guild_queues.get(interaction.guild.id)
        if not queue:
            await interaction.response.send_message("저장할 대기열이 없습니다.", ephemeral=True)
            return
        urls = [track["webpage_url"] for track in queue]
        playlists = self.playlists.setdefault(interaction.guild.id, {})
        playlists[name] = urls
        self.save_playlists()
        await interaction.response.send_message(f"💾 **{name}** 플레이리스트를 저장했습니다. ({len(urls)}곡)", ephemeral=True)
    ...
    @app_commands.command(name="loadplaylist", description="저장된 플레이리스트를 불러옵니다.")
    @app_commands.describe(name="플레이리스트 이름")
    async def load_playlist_slash(self, interaction: discord.Interaction, name: str):
        if not self.is_music_channel(interaction):
            await interaction.response.send_message("먼저 `/setmusicchannel` 명령어로 이 서버의 뮤직 채널을 지정해주세요.", ephemeral=True)
            return
        playlists = self.playlists.get(interaction.guild.id, {})
        urls = playlists.get(name)
        if not urls:
            await interaction.response.send_message("해당 플레이리스트가 없습니다.", ephemeral=True)
            return
        queue = self.guild_queues.setdefault(interaction.guild.id, [])
        for url in urls:
            info = await self.extract_info(url)
            info["requester"] = interaction.user
            queue.append(info)
        await interaction.response.send_message(f"▶️ **{name}** 플레이리스트를 불러왔습니다. ({len(urls)}곡)", ephemeral=True)
        voice_client = self.voice_clients.get(interaction.guild.id)
        if not voice_client or not voice_client.is_connected():
            if not interaction.user.voice or not interaction.user.voice.channel:
    ...
    @app_commands.command(name="listplaylists", description="저장된 플레이리스트 목록을 보여줍니다.")
    async def list_playlists_slash(self, interaction: discord.Interaction):
        playlists = self.playlists.get(interaction.guild.id, {})
        if not playlists:
            await interaction.response.send_message("저장된 플레이리스트가 없습니다.", ephemeral=True)
            return
        desc = "\n".join(f"• {name} ({len(urls)}곡)" for name, urls in playlists.items())
        embed = Embed(title="🎵 저장된 플레이리스트", description=desc, colour=Colour.blue())
    ...
    @app_commands.command(name="deleteplaylist", description="플레이리스트를 삭제합니다.")
    @app_commands.describe(name="플레이리스트 이름")
    async def delete_playlist_slash(self, interaction: discord.Interaction, name: str):
        playlists = self.playlists.get(interaction.guild.id, {})
        if name not in playlists:
            await interaction.response.send_message("해당 플레이리스트가 없습니다.", ephemeral=True)
            return
        playlists.pop(name)
        self.save_playlists()
        await interaction.response.send_message(f"🗑️ **{name}** 플레이리스트를 삭제했습니다.", ephemeral=True)
